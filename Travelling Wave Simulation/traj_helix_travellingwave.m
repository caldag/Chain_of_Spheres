...
% and travelling acoustic fields. Functions called within the script are 
% described below:
%
...
%
% arf_yphi_in: Determines number of spheres and the diameter of the sphere
...
%%%%%%%%%% hasegawa77_tr_calc: This function calculates the radiation force
%%%%%%%%%% function based on Hasegawa's (1977) formulation. This function
%%%%%%%%%% requires:
...
%
% arf_spharr_calc_tr: Calculates the radiation force and torque.
%
% The helix is assumed to be lying along the z- axis.
% Travelling wave is also in the z- direction.

% For each new test, the user should change the following parameters:
% paramlist (line 43), testname (line 53), loop parameter (line 58),
% testparam (line 122). One should also comment the default value placed
% inside the loop for the tested parameter (e.g. line 83 here for Ma sweep).

clearvars;
close all;

paramlist=[1.5]; % Array storing values of parameters changed 
% (assuming multiple simulations are to be carried out).
% In the current case, this parameter corresponds to Ma number 
% (Look at the for loop below)
...
testname='tr_fm20water_n3_Masw_200kpa_test'; % File/folder identifier, used while saving
...
for Ma=paramlist % Ma is the parameter that is swept
    ...
    % (It's commented out because it's the swept parameter)
    ...
    tF=6*TIMEsc; %End time, no acoustics
    tac=0.01*TIMEsc; % Duration of acoustic actuation
    
    dtna = 1/4000*TIMEsc; % Time step when acoustisc is OFF
    tacst=dtna; % Time step when acoustics is turned ON (Use lower values if necessary)
    ...
    [Yphi,n_sph,a_a] = arf_yphi_in(rb0(3),R0,f_a,pamp,hel_length,B,b,nlam,MASSsc,LENGTHsc,TIMEsc);
    %% Simulation
    for t = allt % For all time steps
        ...
        Tmarr(:,k)=Tm;
        [Frad(k), Tac]=arf_spharr_calc_tr(xp,yp,zp,R,hel_length,B,n_sph,a_a,Yphi,pamp,f_a,allt(k),comv,phi,nlam,MASSsc,LENGTHsc,TIMEsc,k);
    ...
    if FIG
            ...
            subplot(311)
            plot(allt(lengthbeforeac:end),UL(1,lengthbeforeac:end)./UL(1,lengthbeforeac),'LineWidth',1.5);ylabel('$U/U_{noac}$');grid on;hold on;
            subplot(312)
            plot(allt(lengthbeforeac:end),UL(2,lengthbeforeac:end)./UL(2,lengthbeforeac),'LineWidth',1.5);ylabel('$V/V_{noac}$');grid on;hold on;
            subplot(313)
            plot(allt(lengthbeforeac:end),UL(3,lengthbeforeac:end)./UL(3,lengthbeforeac),'LineWidth',1.5);ylabel('$W/W_{noac}$');grid on;hold on;
            xlabel('t')
            ...
            subplot(311)
            plot(allt(lengthbeforeac:end),Fradarr(lengthbeforeac:end,1),'LineWidth',1.5);ylabel('$F_{ac,x}$');grid on;hold on;
            subplot(312)
            plot(allt(lengthbeforeac:end),Fradarr(lengthbeforeac:end,2),'LineWidth',1.5);ylabel('$F_{ac,y}$');grid on;hold on;
            subplot(313)
            plot(allt(lengthbeforeac:end),Fradarr(lengthbeforeac:end,3),'LineWidth',1.5);ylabel('$F_{ac,z}$');grid on;hold on;
            xlabel('t')
...
if FIG
    figure(1);set(gcf,'Position',[513 100 706 558])
    ...
    figure(7);set(gcf,'Position',[513 100 706 558]);set(gca,'FontSize',14);
    legh=legend(num2str(paramlist(:)));
    set(legh,'Location','best')
